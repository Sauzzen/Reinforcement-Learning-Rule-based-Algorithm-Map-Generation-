...
class DuelingQNetwork(nn.Module):
    def __init__(self, state_size: Tuple[int, int], action_size: int, input_channels: int = 1):
        super().__init__()
...
class RLAgent:
    ...
    def act(self, state: np.ndarray, action_mask: np.ndarray, deterministic: bool = False) -> int:
        ...
        with torch.no_grad():
            ...
            # Only use valid actions that exist in q_values
            valid_in_range = valid_actions[valid_actions < len(q_values)]
            if len(valid_in_range) == 0:
                return valid_actions[0]
            return valid_in_range[np.argmax(q_values[valid_in_range])]
...
def compute_map_metrics(world: np.ndarray):
    water_frac = np.mean(world == 1)
    land_frac = np.mean(world == 0)
    sand_frac = np.mean(world == 2)
    sand_quality = 1.0 - abs(sand_frac - 0.1)
    largest_land_frac = land_frac
    return {
        "water_frac": float(water_frac),
        "land_frac": float(land_frac),
        "sand_frac": float(sand_frac),
        "sand_quality": float(sand_quality),
        "largest_land_frac": float(largest_land_frac)
    }
